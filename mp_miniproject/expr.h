...
class UnaryExpression : public Expression
{
 ...
 public:
  ...
  virtual ~UnaryExpression() { delete rhs; }
  virtual void assign(map<string, Expression *> & mapping) {
    rhs->assign(mapping);
    return;
  }
...
class PlusExpression : public DoubleExpression
{
 public:
  ...
  PlusExpression(const PlusExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class MinusExpression : public DoubleExpression
{
 public:
  ...
  MinusExpression(const MinusExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class TimesExpression : public DoubleExpression
{
 public:
  ...
  TimesExpression(const TimesExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class DivExpression : public DoubleExpression
{
 public:
  ...
  DivExpression(const DivExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class ModExpression : public DoubleExpression
{
 public:
  ...
  ModExpression(const ModExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class PowExpression : public DoubleExpression
{
 public:
  ...
  PowExpression(const PowExpression & pExpr) :
      DoubleExpression(pExpr.lhs->clone(), pExpr.rhs->clone()) {}
...
class FuncExpression : public Expression
{
 ...
 public:
  ...
  /*  FuncExpression & operator=(const FuncExpression & rhs) {
    if (this != &rhs) {
      delete funcExpr;
      for (map<string, Expression *>::const_iterator it = funcMap.begin(); it != funcMap.end();
           ++it) {
        delete it->second;
      }
  ...
  virtual void assign(map<string, Expression *> & mapping) {
    for (map<string, Expression *>::iterator it = funcMap.begin(), it2 = mapping.begin();
         it != funcMap.end();
         ++it, ++it2) {
      delete it->second;
