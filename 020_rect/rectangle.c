...
rectangle_t intersection(rectangle_t r1, rectangle_t r2) {
  ...
  r1 = canonicalize(r1);
  r2 = canonicalize(r2);
  int x_ol = max(r1.x, r2.x);
  int y_ol = max(r1.y, r2.y);
  int width_ol = min(r1.x + r1.width, r2.x + r2.width) - x_ol;
  int height_ol = min(r1.y + r1.height, r2.y + r2.height) - y_ol;
  if (width_ol < 0 || height_ol < 0) {
    width_ol = height_ol = 0;
  }
  r1.x = x_ol;
  r1.y = y_ol;
  r1.width = width_ol;
